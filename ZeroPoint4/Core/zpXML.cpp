...
void __xmlNodeToString( zpXmlNode* node, zp_int i, zpStringBuffer& buffer )
{
#if XML_FANCY_PRINT
	zpStringBuffer t;
...
zp_bool zpXmlParser::parseFile( const zpString& filename, zp_bool includeSiblings )
{
	...
	while( k < length )
	{
		...
		if( c == xml_open )
		{
			...
			ZP_ASSERT( c == xml_close, "Malformed XML" );
			if( c == xml_close )
			{
