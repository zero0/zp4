...
@ArcCompilerName( "" )
public class CleanToolsCompiler extends ArcCompiler
{
	@Override
	protected List<String> getCompilerParams()
	{
		return Arrays.asList( "cmd", "/c", getProperties().getProperty( "vs.dir" ) + "/vcvarsall.bat", getPlatform().getArch(), "&&", "devenv", getProperties().getProperty( "vs.sln" ), "/Clean", getExecutableMode().name() + "_Tools" );
	}
