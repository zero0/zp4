	...
	, m_count( count )
{
	...
	m_textures.foreachInRange( 0, count, []( ID3D11Texture2D* tex ) {
		tex->AddRef();
	} );
	m_renderTargets.foreachInRange( 0, count, []( ID3D11RenderTargetView* rtv ) {
		rtv->AddRef();
	} );
}
zpDX11RenderTarget::~zpDX11RenderTarget() {
	m_textures.foreachInRange( 0, m_count, []( ID3D11Texture2D* tex ) {
		if( tex ) tex->Release();
	} );
	m_renderTargets.foreachInRange( 0, m_count, []( ID3D11RenderTargetView* rtv ) {
		if( rtv ) rtv->Release();
	} );
