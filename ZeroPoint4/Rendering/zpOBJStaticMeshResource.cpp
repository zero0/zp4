...
zp_bool zpOBJStaticMeshResource::load() {
	...
	while( objFile.readLine( &buff ) > 0 ) {
		...
		if( line[0] == 'v' ) {
			if( line[1] == 'n' ) {
				zp_float x, y, z;
				line.scan( "vn %f %f %f", &x, &y, &z );

				normals.pushBack( zpVector4f( x, y, z ) );
			} else if( line[1] == 't' ) {
				zp_float x, y;
				line.scan( "vt %f %f", &x, &y );

				texCoords.pushBack( zpVector2f( x, y ) );
			} else {
				zp_float x, y, z;
				line.scan( "v %f %f %f", &x, &y, &z );

				verticies.pushBack( zpVector4f( x, y, z ) );
			}
		} else if( line[0] == 'f' ) {
			zp_uint pos = line.indexOf( ' ' ), end = zpString::npos, i = 0;

			zpVertexPositionNormalTexture vnt[3];

			while( ( end = line.indexOf( ' ', pos ) ) != zpString::npos && i < 3 ) {
				zpString face = line.substring( pos, end );

				zp_uint v, t, n;
				face.scan( "%d/%d/%d", &v, &t, &n );
	...
	m_vertexBuffer->create( ZP_BUFFER_TYPE_VERTEX, ZP_BUFFER_BIND_IMMUTABLE, triangles.size() * 3, sizeof( zpVertexPositionNormalTexture ), triangles.begin() );
